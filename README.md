...
VEILLE → OUVERTURE_CANON → TEMPO_DEPART → REMPLISSAGE_POUMON → EN_COURS → TEMPO_ARRIVEE → ARRET_FINAL
                                                                     ↓
                                                             PAUSE_PRESSION (pression perdue)
                                                                     ↓
...
├── securites.c/h           — watchdog SEC-1 / SEC-2 — exécuté en premier dans le tick
├── state_machine.c/h       — 9 états, sous-états poumon, pause pression, urgence NVS
├── calculs_hydraulique.c/h — double interpolation abaque (p_enrouleur + buse + dose)
...
| Namespace | Contenu |
|---|---|
| `irri_machine` | Profil actif, t_vidange, facteur_correction, Kp, modes dégradés |
| `irri_prog` × 5 | Programmes d'arrosage (dose, largeur, buse, pression, temporisations) |
| `irri_state` | Raison du dernier arrêt urgence (persistant après redémarrage) |
...
### 3 onglets (implémenté PR-09)

| Onglet | Contenu |
|---|---|
| **Tableau de bord** | État machine, longueur, vitesse, heure d'arrivée, démarrer/arrêter |
| **Statistiques** | Surface, dose, débit, pression, étage, timings poumon, bilan session |
| **Configuration** | 5 programmes, paramètres machine, mode IRRITESTEUR, OTA firmware |
...
**Deux paramètres sont à mesurer sur machine avant le premier arrosage :**
